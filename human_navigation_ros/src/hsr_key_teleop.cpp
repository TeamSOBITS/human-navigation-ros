...
class HSRKeyTeleop : public rclcpp::Node {
private:
...
private:
  ...
  rclcpp::Publisher<human_navigation::msg::HumanNaviMsg>::SharedPtr pub_msg_;
  ...
  rclcpp::Publisher<human_navigation::msg::HumanNaviGuidanceMsg>::SharedPtr pub_guidance_msg_;
...
: Node("human_navigation_hsr_key_teleop"), 
...
void HSRKeyTeleop::jointStateCallback(const sensor_msgs::msg::JointState::ConstPtr& joint_state) {
...
void HSRKeyTeleop::sendMessage(const std::string &message) {
  ...
  human_navigation::msg::HumanNaviMsg human_navigation_msg;
  human_navigation_msg.message = message;
  pub_msg_->publish(human_navigation_msg);
}
...
void HSRKeyTeleop::operateArm(const double arm_lift_pos, const double arm_flex_pos, const double wrist_flex_pos, const double duration_sec) {
  trajectory_msgs::msg::JointTrajectory joint_trajectory;
...
void HSRKeyTeleop::operateArm(const std::string &name, const double position, const double duration_sec) {
...
void HSRKeyTeleop::operateArmFlex(const double arm_flex_pos, const double wrist_flex_pos) {
...
double HSRKeyTeleop::getDurationRot(const double next_pos, const double current_pos) {
...
int HSRKeyTeleop::run(int argc, char **argv) {
  ...
  pub_msg_ = this->create_publisher<human_navigation::msg::HumanNaviMsg>(pub_msg_to_moderator_topic_name, 10);
...
int main(int argc, char** argv)
{
  ...
  HSRKeyTeleop human_navigation_teleop_key;
  return human_navigation_teleop_key.run(argc, argv);
}
